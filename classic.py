...
if __name__ == "__main__":
    ...
    test_loss, test_acc, y_pred, y_true, y_proba = engine.test(
        net, testloader, loss_fn=criterion, device=DEVICE
    )
    end_test = time.time()
    print(f"Final test set performance:\n\tloss {test_loss}\n\taccuracy {test_acc} %")
    print(
        f"Time performance : Train = {end_train - start_train}seconds  \n\tTest = {end_test - end_train} seconds"
    )
    write_yaml(
        {"Train_time": end_train - start_train, "Test_time": end_test - end_train},
        yaml_path,
